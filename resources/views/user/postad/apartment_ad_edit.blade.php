 ...
 <a href="{{ route('user.user_cart')}}" class="btn btn-primary w-25">Back</a>
    ...
    <div class="form-group" >
      ...
      <select name="category_id" class="form-control  @error('category_id') is-invalid  @enderror">
        <option value="">Choose a Category</option>
                ...
                <option value="{{ $c->id }}" 
                  @if($c->id == $apartment_ad->category_id)
                    selected
                    @endif
      ...
      <select name="subcategory_id" class="form-control  @error('subcategory_id') is-invalid  @enderror">
        <option value="">Choose a Subcategory</option>
            @foreach($subcategories as $c)
                <option value="{{ $c->id }}" 
                  @if($c->id == $apartment_ad->subcategory_id)
                    selected
                    @endif
    ...
    <label class="label-control">Choose Ad Post Type</label>
      <select name="ads_type" class="form-control  @error('ads_type') is-invalid  @enderror">
         <option> {{ $apartment_ad->ads_type }} </option>
         <option>Sale</option>
         <option>Rent</option>
    ...
    <div class="form-group">
      ...
      <div class="form-group" >
        ...
        <textarea  name="description" class="form-control  @error('description') is-invalid  @enderror">{{$apartment_ad->description }}</textarea>
          @error('description')
          <span class="invalid-feedback" role="alert">
              <strong>{{ $message }}</strong>
          </span>
