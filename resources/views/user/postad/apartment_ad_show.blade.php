  ...
  <div class="col-lg-10">
   ...
   <div class="table-responsive"> 
 	<table class="table table-bordered table-striped table-hover datatable student-datatable">
 		...
 		<tr>
 		<tr> <td>ID: {{ $apartment_ad->id }} </td> </tr>
        <tr> <td>Category: {{ $apartment_ad->category->category_name }}</td> </tr>
        <tr><td>Subcategory: {{ $apartment_ad->subcategory->subcategory_name }}</td></tr>
        <tr> <td>County: {{ $apartment_ad->county->county_name }}</td></tr>
        <tr><td>City: {{ $apartment_ad->city->city }}</td></tr>
        <tr> <td>User: {{ $apartment_ad->User->firstname }} {{ $apartment_ad->User->secondname }}</td></tr>
        <tr><td>Title: {{ $apartment_ad->title }}</td></tr>
    <tr><td>Ads_type: {{ $apartment_ad->ads_type }}</td></tr>
     <tr> <td>Real State: {{ $apartment_ad->real_state_type }}</td></tr>
     <tr> <td>Condition: {{ $apartment_ad->condition }}</td></tr>
     <tr> <td>Price: {{ $apartment_ad->price }}</td></tr>
        <tr> <td>Bedroom: {{ $apartment_ad->bedroom }}</td></tr>
        <tr> <td>Bathroom: {{ $apartment_ad->bathroom }}</td></tr>
        <tr> <td>Square Meters: {{ $apartment_ad->square_meters }}</td></tr>
       <tr> <td>Amenities: {{ $apartment_ad->amenities }}</td></tr>
        <tr> <td>Description: {{ $apartment_ad->description }}</td></tr>
        <tr><td>Package: {{ $apartment_ad->Package->package_name }} {{ $apartment_ad->Package->package_amount }}</td></tr>
        <img src="{{ strtok($apartment_ad->photos, ',') }}" width="100">
        </td></tr>
